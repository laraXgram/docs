...
To learn about LaraGram's powerful validation features, let's look at a complete example of validating a form and displaying the error messages back to the user. By reading this high-level overview, you'll be able to gain a good general understanding of how to validate incoming request data using LaraGram:
...
So, what if the incoming request fields do not pass the given validation rules? As mentioned previously, LaraGram will automatically redirect the user back to their previous location. In addition, all of the validation errors and [request input](https://github.com/laraXgram/docs/blob/markdown/requests.md#retrieving-old-input) will automatically be [flashed to the session](https://github.com/laraXgram/docs/blob/markdown/session#flash-data).
...
In addition, you may copy this file to another language directory to translate the messages for your application's language. To learn more about LaraGram localization, check out the complete [localization documentation](https://github.com/laraXgram/docs/blob/markdown/localization.md).
...
You may use the `@error` [temple8](https://github.com/laraXgram/docs/blob/markdown/temple8.md) directive to quickly determine if validation error messages exist for a given attribute. Within an `@error` directive, you may echo the `$message` variable to display the error message:
...
When your application throws a `LaraGram\Validation\ValidationException` exception and the incoming HTTP request is expecting a JSON response, LaraGram will automatically format the error messages for you and return a `422 Unprocessable Entity` HTTP response.
...
If you do not want to use the `validate` method on the request, you may create a validator instance manually using the `Validator` [facade](https://github.com/laraXgram/docs/blob/markdown/facades.md). The `make` method on the facade generates a new validator instance:
...
The field under validation must match the authenticated user's password. You may specify an [authentication guard](https://github.com/laraXgram/docs/blob/markdown/authentication.md) using the rule's first parameter:
...
Instead of providing a literal error message to the `$fail` closure, you may also provide a [translation string key](https://github.com/laraXgram/docs/blob/markdown/localization.md) and instruct LaraGram to translate the error message:
