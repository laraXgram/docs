...
In order to use the `database` queue driver, you will need a database table to hold the jobs. Typically, this is included in LaraGram's default `0001_01_01_000002_create_jobs_table.php` [database migration](https://github.com/laraXgram/docs/blob/markdown/migrations.md); however, if your application does not contain this migration, you may use the `make:queue-table` Commander command to create it:
...
> [!NOTE]
> Job stubs may be customized using [stub publishing](https://github.com/laraXgram/docs/blob/markdown/artisan#stub-customization).
...
In this example, note that we were able to pass an [Eloquent model](https://github.com/laraXgram/docs/blob/markdown/eloquent.md) directly into the queued job's constructor. Because of the `Queueable` trait that the job is using, Eloquent models and their loaded relationships will be gracefully serialized and unserialized when the job is processing.
...
The `handle` method is invoked when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The LaraGram [service container](https://github.com/laraXgram/docs/blob/markdown/container.md) automatically injects these dependencies.

If you would like to take total control over how the container injects dependencies into the `handle` method, you may use the container's `bindMethod` method. The `bindMethod` method accepts a callback which receives the job and the container. Within the callback, you are free to invoke the `handle` method however you wish. Typically, you should call this method from the `boot` method of your `App\Providers\AppServiceProvider` [service provider](https://github.com/laraXgram/docs/blob/markdown/providers.md):
...
> [!WARNING]
> Unique jobs require a cache driver that supports [locks](https://github.com/laraXgram/docs/blob/markdown/cache.md#atomic-locks). Currently, the `memcached`, `redis`, `database`, `file`, and `array` cache drivers support atomic locks. In addition, unique job constraints do not apply to jobs within batches.
...
> [!NOTE]
> If you only need to limit the concurrent processing of a job, use the [WithoutOverlapping](https://github.com/laraXgram/docs/blob/markdown/queues.md#preventing-job-overlaps) job middleware instead.

<a name="encrypted-jobs"></a>
### Encrypted Jobs

LaraGram allows you to ensure the privacy and integrity of a job's data via [encryption](https://github.com/laraXgram/docs/blob/markdown/encryption.md). To get started, simply add the `ShouldBeEncrypted` interface to the job class. Once this interface has been added to the class, LaraGram will automatically encrypt your job before pushing it onto a queue:
...
As you can see, like [listen middleware](https://github.com/laraXgram/docs/blob/markdown/middleware.md), job middleware receive the job being processed and a callback that should be invoked to continue processing the job.
...
> [!NOTE]
> Job middleware can also be assigned to [queueable event listeners](https://github.com/laraXgram/docs/blob/markdown/events.md#queued-event-listeners).

<a name="rate-limiting"></a>
### Rate Limiting

Although we just demonstrated how to write your own rate limiting job middleware, LaraGram actually includes a rate limiting middleware that you may utilize to rate limit jobs. Like [listen rate limiters](https://github.com/laraXgram/docs/blob/markdown/listening.md#defining-rate-limiters), job rate limiters are defined using the `RateLimiter` facade's `for` method.
...
> [!WARNING]
> The `WithoutOverlapping` middleware requires a cache driver that supports [locks](https://github.com/laraXgram/docs/blob/markdown/cache.md#atomic-locks). Currently, the `memcached`, `redis`, `database`, `file`, and `array` cache drivers support atomic locks.
...
Internally, this middleware uses LaraGram's cache system to implement rate limiting, and the job's class name is utilized as the cache "key". You may override this key by calling the `by` method when attaching the middleware to your job. This may be useful if you have multiple jobs interacting with the same third-party service and you would like them to share a common throttling "bucket":
...
> [!NOTE]
> You may also define a `tries` property or `retryUntil` method on your [queued event listeners](https://github.com/laraXgram/docs/blob/markdown/events.md#queued-event-listeners) and [queued notifications](https://github.com/laraXgram/docs/blob/markdown/notifications#queueing-notifications).
...
Often, you know roughly how long you expect your queued jobs to take. For this reason, LaraGram allows you to specify a "timeout" value. By default, the timeout value is 60 seconds. If a job is processing for longer than the number of seconds specified by the timeout value, the worker processing the job will exit with an error. Typically, the worker will be restarted automatically by a [process manager configured on your server](#supervisor-configuration).
...
Without pruning, the `job_batches` table can accumulate records very quickly. To mitigate this, you should [schedule](https://github.com/laraXgram/docs/blob/markdown/scheduling.md) the `queue:prune-batches` Commander command to run daily:
...
> [!NOTE]
> The queue uses the [cache](https://github.com/laraXgram/docs/blob/markdown/cache.md) to store restart signals, so you should verify that a cache driver is properly configured for your application before using this feature.
...
Sometimes your queued jobs will fail. Don't worry, things don't always go as planned! LaraGram includes a convenient way to [specify the maximum number of times a job should be attempted](#max-job-attempts-and-timeout). After an asynchronous job has exceeded this number of attempts, it will be inserted into the `failed_jobs` database table. [Synchronously dispatched jobs](https://github.com/laraXgram/docs/blob/markdown/queues.md#synchronous-dispatching) that fail are not stored in this table and their exceptions are immediately handled by the application.
...
For convenience, you may choose to automatically delete jobs with missing models by setting your job's `deleteWhenMissingModels` property to `true`. When this property is set to `true`, LaraGram will quietly discard the job without raising an exception:
...
To get started, you should schedule the `queue:monitor` command to [run every minute](https://github.com/laraXgram/docs/blob/markdown/scheduling.md). The command accepts the names of the queues you wish to monitor as well as your desired job count threshold:
...
Using the `before` and `after` methods on the `Queue` [facade](https://github.com/laraXgram/docs/blob/markdown/facades.md), you may specify callbacks to be executed before or after a queued job is processed. These callbacks are a great opportunity to perform additional logging or increment statistics for a dashboard. Typically, you should call these methods from the `boot` method of a [service provider](https://github.com/laraXgram/docs/blob/markdown/providers.md). For example, we may use the `AppServiceProvider` that is included with LaraGram:
...
Using the `looping` method on the `Queue` [facade](https://github.com/laraXgram/docs/blob/markdown/facades.md), you may specify callbacks that execute before the worker attempts to fetch a job from a queue. For example, you might register a closure to rollback any transactions that were left open by a previously failed job:
