...
LaraGram Prompts is a PHP package for adding beautiful and user-friendly forms to your command-line applications, with browser-like features including placeholder text and validation.
...
```php
use function LaraGram\Console\Prompts\textarea;
...
```php
use function LaraGram\Console\Prompts\multisearch;
...
```php
use function LaraGram\Console\Prompts\pause;
...
```php
use function LaraGram\Console\Prompts\form;
...
```php
use function LaraGram\Console\Prompts\form;
use function LaraGram\Console\Prompts\outro;
use function LaraGram\Console\Prompts\text;
...
```php
use function LaraGram\Console\Prompts\info;
...
For long running tasks, it can be helpful to show a progress bar that informs users how complete the task is. Using the `progress` function, LaraGram will display a progress bar and advance its progress for each iteration over a given iterable value:

```php
use function LaraGram\Console\Prompts\progress;
...
The callback may also accept the `LaraGram\Console\Prompts\Progress` instance, allowing you to modify the label and hint on each iteration:
...
```php
use function LaraGram\Console\Prompts\clear;
...
If you are not using LaraGram or need to customize when the fallback behavior is used, you may pass a boolean to the `fallbackWhen` static method on the `Prompt` class:
...
If you are not using LaraGram or need to customize the fallback behavior, you may pass a closure to the `fallbackUsing` static method on each prompt class:
